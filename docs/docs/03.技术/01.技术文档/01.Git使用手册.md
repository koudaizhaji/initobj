...
# 示例
git remote add origin git@github.com:koudaizhaji/test.git
```
...
@rabbit-lili ：热爱技术的架构 --> [点这里联系我](https://www.koudaizhaji.top/about/#rabbit-lili)

### 暂存/临时存储

- 当前处于main分支进行了开发，实际需要在main-dev分支开发，又不能直接`git checkout main-dev`
...
#### 恢复指定暂存
- 这里的暂存编号需先通过git stash list来查看后使用
...
#### 本地与远程分支关联
<a id="rabbit-git-01"></a>

```bash
# 将本地的main与远程的main进行关联
git remote add origin https://github.com/koudaizhaji/init-monorepo.git
