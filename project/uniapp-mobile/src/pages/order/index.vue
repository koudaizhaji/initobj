...
<template>
  <view class="swiper-list-container">
    <scroll-view
      ...
      class="tabs-wrapper"
      :scroll-x="true"
      :scroll-into-view="`tabItem${selectedIndex}`"
    ...
    >
      ...
      <swiper-item class="list-item">
        ...
        >
          ...
          <biaofun-loading-more
            :loading="guanzhu.loading"
            :loadall="guanzhu.loadall"
          />
        </scroll-view>
      </swiper-item>
      ...
      <swiper-item class="list-item">
        ...
        >
          <view class="list-content">
            <!-- <text
              v-for="(item, index) in tuijian.list"
              :key="index"
              class="text"
            >
              {{ item.title }}
          ...
          <biaofun-loading-more
            :loading="tuijian.loading"
            :loadall="tuijian.loadall"
          />
        </scroll-view>
      </swiper-item>
    </swiper>
...
export default {
  ...
  data() {
    return {
      ...
      // 推荐数据
      tuijian: {
        refreshing: false, loading: false, loadall: false, pageIndex: 1, pageSize: 20, list: [],
      },
  ...
  methods: {
    ...
    onTapTab(tab, index) {
      ...
      // 切换到的Tab的id
      const { id } = this.tabs[this.selectedIndex];
